/* ═══════════════════════════════════════════════════════════════════════
   SHRI JAGDAMBA INDUSTRY — a very small Firestore client
   ...
   It does exactly two things — read one document, update some fields of
   one document — because that is all the payment endpoints need.
   ═══════════════════════════════════════════════════════════════════════ */
...
function serviceAccount() {
  ...
  try {
    ...
    throw new Error('FIREBASE_SERVICE_ACCOUNT is not valid JSON — paste the whole file contents');
  }
...
/* Read one document. Returns a plain object, or null when it is not there —
...
/* Update only the named fields, leaving the rest of the document alone.
   The update mask is what makes this a merge rather than an overwrite —
...
/* This file lives in api/, so the host will also expose it as a URL. It is
   a helper, not an endpoint, and it holds no secrets — but a bare crash on
